...
/**
 * Converter for Time (MICROSECOND precision) Arrow type
 *
 * @author chenmingliang
 * @date 2026/2/6
 */
public class TimeMicroVectorConvertor extends AbstractMultiTypeConverter<Long> {

    public TimeMicroVectorConvertor(ValueVisitor<Long> visitor, ValueConversionStrategy next) {
        super(visitor, next);
    }
    ...
    public void convertAndSet(ValueVector vector, int index, Object value) {
        if (vector instanceof TimeMicroVector timeMicroVector) {
            timeMicroVector.setSafe(index, this.visit(value));
        } else if (next != null) {
